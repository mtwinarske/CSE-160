...
def read_grid(file_path):
    ...
    grid of pixels.
    """
    # setting the grid as a list in order to be able to call it in
    grid = []
...
def get_pixel_at(pixel_grid, i, j):
    ...
    """
# if there is no column i or j, will return the pixel at that location.
...
def blur(pixel_grid):
    ...
    for row in pixel_grid:
        ...
        j = 0
        for spot in row:
            average = average_of_surrounding(pixel_grid, i, j)
...
write_image(output_image_filename, blurred_grid)
write_grid(output_grid_filename, blurred_grid)
...
# I worked on this assignment alone, and only referred to the homework
