...
def nano_suitable(gc_content):
    ...
    DNA nanotechnology
    '''
    if gc_content > 0.50 and gc_content < 0.60:
        return True
    else:
        return False
...
# Number GC & AT nucleotides seen so far.
gc_count = 0
at_count = 0

# Number of G's, C's, A's, and T's counted
g_count = 0
c_count = 0
a_count = 0
t_count = 0
...
gc_content = (gc_count) / (g_count + c_count + a_count + t_count)

at_content = (at_count) / (g_count + c_count + a_count + t_count)
...
print('Sum of G+C+A+T counts:', g_count + c_count + a_count + t_count)
